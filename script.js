...
const reserveRoomSelect = document.getElementById("reserveRoom");
const guestsSelect = document.getElementById("guests");

// [객실별 최대 인원 옵션 동적 갱신]
function updateGuestOptions() {
  const guestLimits = { "Ocean A": 4, "Garden B": 3, "Suite C": 8 };
  const maxGuests = guestLimits[reserveRoomSelect.value] || 4;
  guestsSelect.innerHTML = "";

  for (let count = 2; count <= maxGuests; count++) {
    const option = document.createElement("option");
    option.value = `${count}명`;
    option.textContent = `${count}명`;
    guestsSelect.appendChild(option);
  }
}

reserveRoomSelect.addEventListener("change", updateGuestOptions);
updateGuestOptions();

// [실시간 예약 폼 제출 -> reservation.php 로 전송 (서버에서 날짜/중복 검증 후 DB 저장)]
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    formData.append("checkIn", inDate);
    formData.append("checkOut", outDate);

    try {
      const res = await fetch("reservation.php", {
        method: "POST",
        body: formData,
      });
      const data = await res.json();
